...
class Drafts extends React.Component {
  ...
  parseData($data) {
    if (
      ...
      try {
        ...
        console.error(e);
        this.boundSnackFailureDismiss("Failed parsing data [JSON.parse()].");
      }
  }
  ...
  componentDidMount() {
    ...
    let $data = document.querySelector('script#data[type="application/json"]');
  ...
  render() {
    ...
    return loaded === true ? (
      <div>
        <Grid container direction="column" spacing={4} key="grid_normal">
          <Grid item xs={true} className={classes.grid_fabs_item}>
            <Tooltip title="Add new draft" aria-label="Add new draft">
